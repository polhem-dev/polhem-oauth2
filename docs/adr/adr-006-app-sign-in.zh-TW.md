...
- 所有被接受的組合，都在不帶 client secret 的情況下換到 token。Facebook 的這一點屬於實測到的行為：它的手動流程文件把 client secret 列為必要、
  ...
  2026-09-26 在 iOS 模擬器重測，Google 的 iOS client 仍不帶 client secret 就登入成功；同一天 ADR-004 的桌面 client 不帶 secret 則被拒：
  Google 依 client 類型決定是否要求 secret。
...
### 2026-09-26 iOS 與 Android 重測

以 1.2.0 之後 `main` 的函式庫，透過 `samples/OAuthMaui` 在 iPhone 17 Pro 模擬器（iOS 26.5）與 Android 15 模擬器登入，
後端為 `samples/OAuthAspNetCore`。中轉前，以 `xcrun simctl keychain <device> add-root-cert` 把後端的開發憑證加進 iOS 模擬器信任的根憑證；
Android 則比照 `tests/Polhem.OAuth2.DeviceTests/scripts/prepare-android-emulator.sh` 加進模擬器的使用者憑證，並執行 `adb reverse tcp:7032 tcp:7032`。

| Provider | 直連，iOS | 中轉，iOS | 直連，Android | 中轉，Android |
|----------|-----------|-----------|---------------|---------------|
| Google | 成功，未帶 client secret | 成功 | 不可行（見背景） | 成功 |
| Microsoft Entra ID | 成功 | 成功 | 成功 | 成功 |
| Auth0 | 成功 | 成功，網頁應用程式先經過 Auth0 的授權同意頁 | 成功 | 成功，同樣先經過授權同意頁 |
| Okta | 成功 | 成功 | 成功 | 成功 |
